...
def add_L2_term( objective, f='f', df='df' , gtol=1.1e-5 ):
    '''
    ...
    def optimize_L2( X, Y, mu, **optimize_params):
        ...
        opter = optimizer( objective , init_params=X , f='L2_f' , df='L2_df' , 
                           maxiter=100, gtol=gtol, disp=0,  **optimize_params)
        return opter()
...
#def groupL1_L2( rho , modulo=0 ):  TODO
#    '''
#    Provides object with methods:
#        f(X)  = rho * ||X||_1 
#        df(X) = a subgradient of f
...
def falms( init_params , F, G, mu=0.1, maxiter=200, ftol=2e-7, callback=None,verbose=True):
    ...
    for j in range(maxiter):
        ...
        try:
            ...
            excepted = True
        if excepted or objval>=objval_old:
            mu = mu/2
            falmer = oldfalmer
        else:
            objval_old  = objval
    return F.unflat( falmer[1] )